...
export default function AccountsPayable() {
  ...
  // Queries
  const { data: apData, isLoading: apLoading } = trpc.financial.getAccountsPayable.useQuery({
    daysBack,
  });

  const { data: apSummary, isLoading: summaryLoading } = trpc.financial.getAPSummary.useQuery();

  const isLoading = apLoading || summaryLoading;
      ...
      <div className="container mx-auto max-w-7xl space-y-8">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-gray-300 mt-1">Gestão de pagamentos a fornecedores e operadoras</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="bg-[#1a2a4a] border-white/10">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-yellow-400">{apSummary?.pendingCount || 0}</div>
              <p className="text-xs text-gray-400 mt-1">{formatCurrency(apSummary?.totalPending)}</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-[#1a2a4a] border-white/10">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-400">{apSummary?.overdueCount || 0}</div>
              <p className="text-xs text-gray-400 mt-1">{formatCurrency(apSummary?.totalOverdue)}</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-[#1a2a4a] border-white/10">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-400">{apSummary?.paidCount || 0}</div>
            </CardContent>
          </Card>
        ...
        <Card className="bg-[#1a2a4a] border-white/10">
          ...
          <CardContent>
            {apData && apData.records && apData.records.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10">
                      ...
                      <th className="text-center py-3 px-4 text-gray-300">Pagamento</th>
                    </tr>
                  </thead>
                  <tbody>
                    {apData.records.map((record: any, idx: number) => (
                      <tr key={idx} className="border-b border-white/5 hover:bg-white/5">
                        <td className="py-3 px-4 text-white font-mono text-xs">{record.invoiceId}</td>
                        <td className="py-3 px-4 text-gray-300">{record.supplierName}</td>
              ...
              <div className="text-center py-8 text-gray-400">
                Nenhum registro de contas a pagar encontrado
              </div>
            )}
          </CardContent>
        </Card>
